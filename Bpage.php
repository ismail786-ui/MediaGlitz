...
<body>
    ...
    echo "<center><h2>Booking List</h2><p>No of Booking : " . $no_of_booking . "</p></center>";
    echo "<center><p> Pages : " . ($start_limit + 1) . " To " . ($start_limit  +  $no_of_booking_per_page) . "</p></center>";
    // $sql = "select * from booking limit $start_limit,$no_of_booking_per_page";
    $sql = "select * from booking ORDER BY id DESC limit $start_limit,$no_of_booking_per_page";
    $res = $conn->query($sql);
    while ($row = $res->fetch_assoc()) {
        echo "<p class='Book' >{$row['id']}--{$row['customername']}--{$row['eventname']}--{$row['eventtype']}--{$row['Wevent']}--{$row['elocation']}--{$row['eventcity']}--{$row['state']}--{$row['people']}--{$row['date']}--{$row['time']}--{$row['hour']} </p>";
    }
    for ($i = 1; $i <= $no_of_pages; $i++) {
        if ($page == $i) {
            echo "<a href='./Bpage.php?page={$i}' class='link aaa ' >{$i}</a>";
        } else {
            echo "<a href='./Bpage.php?page={$i}' class='link' >{$i}</a>";
        }
    }
